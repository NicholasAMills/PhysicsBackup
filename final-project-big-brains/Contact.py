...
def bounce(contact_data, restitution=0, friction=0, static_friction=None):
    if contact_data is not None:
        ...
        if d > 0:
            ...
            if vn < 0:
                ...
                if jt * (vt + 0.5 * v) < 0:
                    kinetic_friction = vn/(Mtt - mew * Mnt)
                else:
                    mew = -mew
                    kinetic_friction = vn/(Mtt - mew * Mnt)


                j = jn * n.hat() + jt * t_hat  # j for static friction
                if j is None:
                    j = kinetic_friction

                # print("j is: ", j)
                if j is Vec2(0, 0):
                    j = (-m * (1 + restitution) * vn) * n  # j for no friction
